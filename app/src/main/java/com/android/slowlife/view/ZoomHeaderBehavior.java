...
public class ZoomHeaderBehavior extends CoordinatorLayout.Behavior<View> {
    ...
    private void changeView(View child, View dependency, ViewPager viewPager, int i) {
        ...
        View numTxt = v.findViewById(R.id.tv_num);
        View origText = v.findViewById(R.id.tv_origprice);
        TextView currText = (TextView) v.findViewById(R.id.tv_currentprice);
        currText.getPaint().setFlags(Paint. STRIKE_THRU_TEXT_FLAG); //中划线
        View left = null;
            ...
            if (progress > 0) {
                ...
                bottom.offsetLeftAndRight(
                        (int) (target.getWidth() / 2 - target.getWidth() * (1 + progress) / 2
                                + Config.MARGIN_LEFT_RIGHT - bottom.getX()));

                //让button x 对齐image 右侧
                buyButton.offsetLeftAndRight(
                        (int) (target.getWidth() / 2 + target.getWidth() * (1 + progress) / 2
                                - buyButton.getWidth() - 30
                                - Config.MARGIN_LEFT_RIGHT
                                - buyButton.getX()));

//                //按钮与文字居中
//                buyButton.offsetTopAndBottom((int) (bottom.getY()
//                        + ((origText.getY() + origText.getHeight()) - nameTxt.getY())
